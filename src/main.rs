...
fn challenge6() {
    ...
    let klmin: usize = 2;
    let klmax: usize = 40;
    // the number of blocks to pair and test
    let testblocks: usize = 3;
    ...
    impl fmt::Display for Resultat {
        fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
            // Use `self.number` to refer to each positional data point.
            write!(f, "Length: {}\tAvg: {}\n", self.length, self.avgdistance)
        }
    }
    ...
    for kl in klmin..=klmax {
        ...
        for it in (0..testblocks).combinations(2) {
        ...
        resultats.push(Resultat {
            // length: cumulator.len() as u32,
            length: kl as u32,
            avgdistance: cumulator.avg() / kl as f32,
        });

        println!(
            "Key length: {} - length: {} - sum: {} - average: {:.2}",
            kl,
            cumulator.len(),
            cumulator.sum(),
